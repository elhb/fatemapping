...
def vcfParser(infilename,subsetSize=None,logfile=False):
	...
	if logfile: logfile.write('#LOGMSG#'+time.strftime("%Y-%m-%d:%H:%M:%S",time.localtime())+'#'+str(AnalysisPipe.masterPid)+'# Loading vcf to memory, my id is '+str(os.getpid())+'.\n')
	...
	with open(infilename) as infile:
	    for line in infile:
		if not subsetSize: progress.update()
		if subsetSize and (sum([len(self.variations[chrom]) for chrom in self.variations]) >= subsetSize or toQcounter >= subsetSize):
		...
		if variation.passFilter == 'PASS' and not variation.isIndel and variation.varQual >= 30:
		    yield variation		    
		    if subsetSize: progress.update()
		    toQcounter+=1
		    if variation.chrom != lastChomosomeRead and logfile: logfile.write('#LOGMSG#'+time.strftime("%Y-%m-%d:%H:%M:%S",time.localtime())+'#'+str(AnalysisPipe.masterPid)+'# started loading variations from '+str(variation.chrom)+'.\n')
		    lastChomosomeRead = variation.chrom
	if logfile: logfile.write('#LOGMSG#'+time.strftime("%Y-%m-%d:%H:%M:%S",time.localtime())+'#'+str(AnalysisPipe.masterPid)+'# vcf file loaded.\n')
